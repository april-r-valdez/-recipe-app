...
function InputIngredient() {
  ...
  const handleSearch = () => {
    const ingredientNames = ingredients
      ...
      .join(",");
    navigate(
      `/searchByIngredients?searchType=${"byIngredient"}&ingredients=${ingredientNames}&glutenFree=${isGlutenFree}&dairyFree=${isDairyFree}&vegan=${isVegan}`
    );
  };
  ...
  const modalTitle = <FcCameraIdentification />;
  const modalBody = (
    <CameraModule
    ...
    const handleDownload = async (e) => {
      ...
      try {
        ...
        if (!snapshot.empty) {
          ...
          setIngredientList(pantry.data().ingredients);
        }
      } catch (error) {
  ...
  return (
    ...
    >
      <div className="row">
        <label className="fw-medium text-warning text-start">
          SEARCH RECIPE
        </label>
        <div className="input-group mb-3">
          <span
            className="input-group-text fw-medium text-warning"
            id="basic-addon3"
          >
            USING
          </span>
      ...
      <div
        className={`collapse ${isHovered ? "show" : ""}`}
        id="ingredientSearchCollapse"
      >
        <div className="row">
          <div className="col">
